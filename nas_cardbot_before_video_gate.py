...
async def fetch_share_video_files(share_url: str, max_files: int = 3000):
    ...
    try:
        ...
        data = await client.share_snap_app({
            "share_code": code, "receive_code": rc or "", "cid": 0,
            "limit": 1000, "offset": 0
        }, async_=True)
        inner = data.get("data") if isinstance(data.get("data"), dict) else data
        root_list = inner.get("list") or []
        root_name = (root_list[0].get("fn") if root_list else "") or ""

        vids = []

        async def walk(cid):
            d = await client.share_snap_app({
                "share_code": code, "receive_code": rc or "", "cid": cid,
                "limit": 1000, "offset": 0
            }, async_=True)
            inn = d.get("data") if isinstance(d.get("data"), dict) else d
            for it in (inn.get("list") or []):
                fn = it.get("fn") or ""
                fc = it.get("fc")
...
async def fetch_share_top_folders(share_url: str) -> list[dict]:
    """Return top-level folders when a share is a multi-title collection."""
    code, rc = _parse_share_url(share_url)
    if not code:
        return []
    try:
        ...
        for item in inner.get("list") or []:
            if item.get("fc") in (0, "0") or bool(item.get("is_dir")):
                folders.append({"id": str(item.get("fid") or ""), "name": str(item.get("fn") or "")})
        markers = {parse_tmdbid_marker(item["name"]) for item in folders}
        markers.discard(None)
        non_season = [item for item in folders if not re.fullmatch(r"Season\s*\d+|S\d+", item["name"], re.I)]
        return folders if len(folders) > 1 and (len(markers) > 1 or len(non_season) > 1) else []
    except Exception as exc:
        logger.warning(f"获取115分享顶层目录失败: {exc}")
        return []
...
def _is_unrecoverable_reason(reason: str) -> bool:
    ...
    if ("违规文件" in text or "审核未通过" in text
            or "链接包含违规内容" in text or "未检测到任何文件" in text
            or "未检测到任何视频" in text or "空文件夹" in text):
        return True
    # 分享已取消：用户或系统取消了分享
    if "分享已取消" in text or "已取消" in text:
        return True
    # 分享不存在/已删除
    if "分享不存在" in text or "已删除" in text or "not found" in text.lower():
        return True
    return False
...
def _load_retry_queue() -> None:
    ...
    try:
        with open(RETRY_STATE_FILE, "r", encoding="utf-8") as fp:
            data = json.load(fp)
        _RETRY_QUEUE = data if isinstance(data, list) else []
        migrated = False
...
def _retry_queue_has_url(url: str) -> bool:
    """避免同一原始分享在审核队列中重复创建转存目录/分享。"""
    return any(str(item.get("url", "")) == str(url) for item in _RETRY_QUEUE)
...
async def handle_message(message: types.Message):
    ...
    # 仅拦截“当前正在处理”的链接；处理结束立即释放。
    # 这样同一分享完成后可以再次发送（例如需要重新转存/重命名），
    # 同时避免用户在同一任务尚未完成时重复触发并发转存。
    real = []
    for u in links:
        if u in _processing:
            continue
        _processing[u] = asyncio.get_event_loop().time()
        real.append(u)
    if not real:
        try:
            await message.answer("⚠️ 该链接当前正在处理中，已跳过并发重复任务。")
        except Exception as e:
            logger.warning(f"回复失败（不影响）: {e}")
        return
    ...
    for url in real:
        try:
            ...
            await process_one(url, user_name, message, status, grouped_urls=grouped)
        except Exception as e:
            ...
            # 任务无论成功/失败都释放占用，允许用户之后重新发送同一链接。
            _processing.pop(url, None)
...
async def _rename_saved(svc, to_cid, title_cn: str, year: str, tmdb_id, suffix: str = "Cxuan",
    ...
    返回 (ok, fail, total, new_cid)，new_cid 非空表示新建了文件夹（分享目标需切换）。
...
async def save_rename_share(url: str, metadata: dict, title_cn: str, year: str, tmdb_id=None,
    ...
    # 分享根无文件夹时 _rename_saved 自动建了新文件夹，分享目标要切到它
    if new_cid:
        save_res = dict(save_res)
        save_res["to_cid"] = new_cid
...
async def _multi_title_metadata(folder_name: str, videos: list[dict]) -> dict:
    names = [item.get("name", "") for item in videos]
    sample = _select_source_name(names, folder_name, "") or folder_name
    data = await _pipeline_metadata(sample)
    marker_id = parse_tmdbid_marker(folder_name)
    if marker_id and marker_id != data.get("tmdb_id"):
        parsed = parse_filename(sample)
        episode = extract_episode(sample)
        info = await tmdb_search(
            parsed.get("title") or folder_name, parsed.get("year", ""),
            episode[0] if episode else None, tmdb_id=marker_id, source_name=sample,
        ) if TMDB_API_KEY else None
        if info:
            data.update({
                "display_title": info.get("title") or data["display_title"],
                "year": info.get("year") or data["year"],
                "tmdb_id": info.get("tmdb_id") or marker_id,
                "poster_url": info.get("poster_url", ""),
                "genres": info.get("genres", "暂无"),
                "tmdb_rating": info.get("rating", "暂无评分"),
                "overview": info.get("overview", ""),
            })
    data["episode"] = infer_episode_range(names, folder_name) or data.get("episode", "")
    data["audio"] = infer_audio_codec(names)
    data["size_text"] = ""
    return data


async def save_split_multi_title_share(url: str, metadata: dict, expected_top_count: int) -> dict:
    """Save once, then rename/share each title group independently."""
    svc = get_svc()
    save_res = await _save_share_with_retry(svc, url, metadata)
    if not save_res or save_res.get("status") != "success":
        return save_res or {"status": "error", "message": "多剧合集转存失败"}
    to_cid = save_res.get("to_cid")
    if not to_cid:
        return {"status": "error", "message": "多剧合集转存后未取得任务目录"}

    top = await _get_ready_dir_items(
        svc, to_cid, min_wait=8, max_wait=90, expected_count=expected_top_count,
    )
    folders = [item for item in top if item.get("is_dir")]
    if len(folders) < 2:
        return {"status": "error", "message": "转存后未检测到多个剧集目录", "cleanup_cid": str(to_cid)}

    groups = {}
    for folder in folders:
        marker = parse_tmdbid_marker(folder["name"])
        title_key = re.sub(r"\s*[({]\s*tmdb(?:id)?[-:]?\d+\s*[)}]", "", folder["name"], flags=re.I)
        title_key = re.sub(r"\s*\(\d{4}\)\s*$", "", title_key).strip().casefold()
        groups.setdefault(f"tmdb:{marker}" if marker else f"title:{title_key}", []).append(folder)

    results = []
    for index, folders_in_group in enumerate(groups.values(), 1):
        videos = []
        for folder in folders_in_group:
            videos.extend(await _pipeline_collect_videos(svc, int(folder["id"])))
        if not videos:
            logger.warning("跳过无视频的多剧目录: %s", folders_in_group[0]["name"])
            continue
        item = await _multi_title_metadata(folders_in_group[0]["name"], videos)
        canonical_folder = build_folder_name(item["display_title"], item["year"], item.get("tmdb_id"))
        for folder_index, folder in enumerate(folders_in_group, 1):
            target_name = canonical_folder if folder_index == 1 else f"{canonical_folder} ({folder_index})"
            rename_res = await svc.client.fs_rename((folder["id"], target_name), async_=True)
            if isinstance(rename_res, dict) and rename_res.get("state") is False:
                logger.warning("多剧目录重命名失败: %s | %s", folder["name"], rename_res)
            await _rename_video_tree(
                svc, folder["id"], item["display_title"], item["year"], item.get("tmdb_id"), suffix="Cxuan",
            )
        folder_ids = [str(folder["id"]) for folder in folders_in_group]
        share_link = await svc._share_fids_direct(folder_ids)
        if not share_link:
            item.update({"status": "error", "message": "永久分享链接生成失败", "cleanup_cids": folder_ids})
            results.append(item)
            continue
        ...
        item.update({
            ...
            "message": reason,
            "cleanup_cids": folder_ids,
        })
        results.append(item)
        logger.info(
            "✅ 多剧合集已拆分永久分享: %s (%d/%d)",
            item["display_title"], index, len(groups),
        )
        await asyncio.sleep(1)
    return {"status": "success", "items": results, "task_cid": str(to_cid)}


def _queue_split_pending(url: str, user_name: str, user_id, item: dict) -> None:
    _RETRY_QUEUE.append({
        "url": url, "user_name": user_name,
        "display_title": item["display_title"], "year": item.get("year", ""),
        "tmdb_id": item.get("tmdb_id"), "poster_url": item.get("poster_url", ""),
        "genres": item.get("genres", "暂无"), "tmdb_rating": item.get("tmdb_rating", "暂无评分"),
        "douban": item.get("douban", ""), "overview": item.get("overview", ""),
        "quality": item.get("quality", ""), "source": item.get("source", ""),
        "episode": item.get("episode", ""), "encode": item.get("encode", ""),
        "audio": item.get("audio", ""), "size_text": item.get("size_text", ""),
        "user_id": user_id, "retry_count": 0,
        "pending_share_link": item.get("share_link", ""),
        "pending_cleanup_cids": item.get("cleanup_cids", []),
        "pending_cleanup_name": item["display_title"],
        "fail_reason": item.get("message") or "新分享正在审核中",
        "split_multi_title": True,
    })


async def _process_multi_title_share(url: str, user_name: str, message, status, top_folders: list[dict]):
    user_id = message.from_user.id if message.from_user else None
    queued = sum(1 for item in _RETRY_QUEUE if item.get("url") == url and item.get("split_multi_title"))
    if queued:
        await status.edit_text(f"⏳ 这个多剧合集已有 {queued} 个拆分链接在审核队列中，请等待自动发布。")
        return
    async with _SAVE_LOCK:
        result = await save_split_multi_title_share(
            url,
            {"description": user_name or url, "full_text": user_name or url, "command_mode": "share"},
            len(top_folders),
        )
    if not result or result.get("status") != "success":
        reason = (result or {}).get("message") or "多剧合集处理失败"
        await status.edit_text(f"❌ 多剧合集处理失败：{reason}")
        await message.reply(f"❌ 处理失败：{url}\n原因：{reason}")
        return

    ready_count = pending_count = failed_count = 0
    for item in result.get("items", []):
        if item.get("status") == "pending":
            _queue_split_pending(url, user_name, user_id, item)
            pending_count += 1
            continue
        ...
        try:
            sent = await _render_and_send_card(
                message.bot, int(TG_CHANNEL_ID) if str(TG_CHANNEL_ID).lstrip("-").isdigit() else 0,
                ...
                share_link=item["share_link"], overview=item["overview"], poster=poster,
            )
        finally:
            del poster
        if sent:
            ready_count += 1
            for cid in item.get("cleanup_cids", []):
                _schedule_cleanup(cid, item["display_title"], item["share_link"])
        else:
            failed_count += 1
    _save_retry_queue()
    await status.edit_text(
        f"✅ 多剧合集已按剧名拆分：已发布 {ready_count} 部，审核中 {pending_count} 部，失败 {failed_count} 部。"
    )
    await message.reply(
        f"✅ 已按剧名拆分处理，共识别 {len(result.get('items', []))} 部；"
        f"审核中的链接完成后会自动发布。"
    )
        ...
        try:
            ...
            try:
                ...
                if ok:
                    ...
                    cleanup_cids = item.get("pending_cleanup_cids") or ([res.get("cleanup_cid")] if res.get("cleanup_cid") else [])
                    if sent:
                        for cleanup_cid in cleanup_cids:
                            _schedule_cleanup(
                                cleanup_cid, res.get("cleanup_name") or item["display_title"], share_link,
                            )
                    if item.get("user_id"):
                        try:
            ...
            if _RETRY_MAX > 0 and item["retry_count"] >= _RETRY_MAX:
                ...
                cleanup_cids = item.get("pending_cleanup_cids") or (
                    [item.get("pending_cleanup_cid")] if item.get("pending_cleanup_cid") else []
                )
                if item.get("pending_share_link"):
                    for cleanup_cid in cleanup_cids:
                        _schedule_cleanup(
                            cleanup_cid,
...
def _pipeline_recover_processing() -> None:
    ...
    for path in _pipeline_task_paths("processing"):
        pending = path.with_name(path.name.replace(".processing.json", ".failed.json"))
        try:
...
async def _pipeline_process_folder_task(task: dict, svc) -> dict:
    ...
    task["size"] = sum(int(v.get("size", 0) or 0) for v in videos)
    ...
    if not task.get("renamed_name"):
        ...
        logger.info("✅ 闭环剧集目录重命名完成: %s -> %s", relative, folder_name)
        await _rename_video_tree(
    ...
    if not share_link:
        ...
        logger.info("✅ 闭环剧集永久分享已生成: %s", _safe_log_url(share_link))
    ...
    _schedule_cleanup(folder_id, task.get("renamed_name") or task["display_title"], share_link, service="pipeline", delay=0)
    logger.info("✅ 123→115 剧集合并闭环完成: %s（%d 集）", task["display_title"], len(videos))
    return task
...
async def _pipeline_process_task(task: dict) -> dict:
    ...
    # The mover can emit the same file twice. If the first event already
    # completed and cleanup removed the source, close the duplicate without
    # creating a second share or channel card.
    ...
    if not task.get("renamed_name"):
        ...
        logger.info("✅ 闭环重命名完成: %s -> %s", filename, renamed_name)
    ...
    if not share_link:
        ...
        task["share_link"] = share_link
        logger.info("✅ 闭环永久分享已生成: %s", _safe_log_url(share_link))
    ...
    if task.get("card_sent"):
        if not task.get("cleanup_scheduled") and task.get("file_id"):
            _schedule_cleanup(
                task["file_id"], task.get("renamed_name") or task["display_title"],
                task.get("share_link", ""), service="pipeline", delay=0,
            )
            task["cleanup_scheduled"] = True
        return task
    poster = await fetch_poster_bytes(task["poster_url"]) if task.get("poster_url") else None
    try:
        ...
        sent = await _render_and_send_card(
            ...
            tmdb_rating=task["tmdb_rating"], douban_rating=task["douban"],
            quality=task["quality"], source=task["source"],
            size_text=format_size(int(task.get("size", 0) or 0)),
            episode=task["episode"], encode=task["encode"], audio=task["audio"],
            share_link=share_link, overview=task["overview"], poster=poster,
        )
    finally:
        del poster
    if not sent:
            ...
            for path in candidates:
                try:
                    ...
                    if task.get("kind") != "folder" and _pipeline_has_folder_task(task.get("relative_path", "")):
                        # An older per-file event must not race the aggregate
                        # series task created by the mover.
                        continue
                    if task.get("kind") == "folder":
                        quiet_until = float(task.get("last_seen_at", task.get("created_at", 0)) or 0) + PIPELINE_GROUP_DELAY
                        if quiet_until > time.time():
                            task["next_retry_at"] = quiet_until
                            _pipeline_write(path, task)
                            continue
                    processing = path.with_name(path.name.replace(path.suffixes[-2] + path.suffix, ".processing.json"))
                ...
                try:
                    async with _SAVE_LOCK:
                        result = await _pipeline_process_task(task)
                    ...
                    processing.unlink(missing_ok=True)
                    logger.info("⏳ 闭环分享审核中，稍后继续: %s", task.get("relative_path", ""))
                except Exception as exc:
                    ...
                    _pipeline_write(failed, task)
                    processing.unlink(missing_ok=True)
                    logger.warning("⚠️ 闭环任务失败，自动重试: %s | %s", task.get("relative_path", ""), exc)
        except Exception as exc:
...
async def process_one(url: str, user_name: str, message: types.Message, status: types.Message,
    ...
    if is_mode1:
        ...
        try:
            video_names, root_name = await fetch_share_video_files(url)
        except Exception as e:
            logger.warning(f"fetch_share_video_files 异常: {e}")
        try:
            top_folders = await fetch_share_top_folders(url)
        except Exception as e:
            logger.warning(f"fetch_share_top_folders 异常: {e}")
        if not top_name or SHARE_PLACEHOLDER_KW.search(top_name):
            ...
            return
        # 违规分享有时仍能列出目录，但实际没有任何可转存的视频文件。
        # 这类资源不应继续转存、建空目录或进入无限重试队列。
        if not video_names and root_name:
            reason = "原始分享未检测到任何视频文件（可能仅剩文件夹或内容已被115违规屏蔽）"
            logger.warning(f"🚫 {reason}，直接宣布失败: {_safe_log_url(url)}")
            try:
                if status:
                    await status.edit_text(f"❌ 资源处理失败：{reason}")
                await message.reply(f"❌ 处理失败：{url}\n原因：{reason}")
            except Exception as e:
                logger.warning(f"发送空视频资源失败通知失败: {e}")
            return {"status": "failed", "dedup": False, "terminal": True}
        if top_folders and not test_mode:
            logger.info("📚 检测到多剧合集，按顶层剧名拆分处理: %d 个目录", len(top_folders))
            await _process_multi_title_share(url, user_name, message, status, top_folders)
            return
...
async def main():
    ...
    # 启动后台 worker：失败重试 + 挂载闭环 + 发送成功20分钟后移入回收站
